  ...
  return (
    <section className="py-16 md:py-24">
      <Container>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-4 md:gap-3">
          {tiles.map(({ label, href, Icon }) => (
            <Link
              key={label}
              href={href}
              className="group flex flex-col items-center justify-center gap-4 aspect-square border border-brown/10 bg-white px-3 text-center transition-all duration-300 hover:border-gold hover:shadow-[0_18px_40px_-20px_rgba(58,40,24,0.35)] hover:-translate-y-0.5"
            >
              <span className="flex h-12 w-12 items-center justify-center rounded-full border border-gold/40 text-gold-dark transition-colors duration-300 group-hover:bg-brown group-hover:border-brown group-hover:text-yellow-warm">
