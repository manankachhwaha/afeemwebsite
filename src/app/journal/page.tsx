  ...
  return (
    <>
      ...
      <section className="py-16 md:py-24">
        <Container className="grid sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-12">
          {journalPosts.map((p) => (
            <Link key={p.slug} href={`/journal/${p.slug}`} className="group flex flex-col gap-4">
              <Visual label={p.bucket} ratio="aspect-[4/3]" className="group-hover:opacity-90 transition-opacity" />
              <div>
